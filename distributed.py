...
import random
random.seed(int(os.environ.get('SEED', 0)) or None)
...
class DistributedDataParallel(Module):
    ...
    def _make_param_hook(self, param, device_idx):
        ...
        def distributed_data_parallel_hook(*unused):
            ...
            if int(os.environ.get('CNAT_COMPRESS', 0)) and param.grad.data.dtype == torch.float32 and random.random() < 0.4:
                # compress
                with torch.no_grad():
                    bucket[bucket_offset] = cnat(param.grad.data)
            else:
